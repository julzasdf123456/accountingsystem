...
public class HomeController implements Initializable {
    ...
    //BUDGETING
    public JFXButton cob, rv, po, user_po, user_cob, user_rv, manage_app;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (ActiveUser.getUser().can("manage-po"))
            NavMenuHelper.addMenu(navMenuBox, user_po, homeStackPane);

        if (ActiveUser.getUser().can("prepare-po"))
            NavMenuHelper.addMenu(navMenuBox, po, homeStackPane);
        ...
        if(ActiveUser.getUser().can("manage-po"))
            DrawerMenuHelper.setMenuButtonWithViewAndSubMenu(user_po, new FontIcon("mdi2v-view-dashboard-variant"), drawerMenus, user_po.getText(), contentPane, "budgeting/budgeting_user_po_list.fxml", subToolbar, null, title);

        if(ActiveUser.getUser().can("prepare-po"))
            DrawerMenuHelper.setMenuButtonWithViewAndSubMenu(po, new FontIcon("mdi2c-cash-usd"), drawerMenus, po.getText(), contentPane, "budgeting/budgeting_po.fxml", subToolbar, null, title);
        ...
        if(ActiveUser.getUser().can("manage-finance")) {
            ...
            DrawerMenuHelper.setMenuButtonWithViewAndSubMenu(checkVoucher, new FontIcon("mdi2c-checkbox-blank-circle-outline"), drawerMenus, checkVoucher.getText(), contentPane, "finance/checkVoucher_layout.fxml", subToolbar, null, title);
            DrawerMenuHelper.setMenuButtonWithViewAndSubMenu(journalVoucher, new FontIcon("mdi2f-file"), drawerMenus, journalVoucher.getText(), contentPane, "finance/checkVoucher_layout.fxml", null, null, title);
        }
